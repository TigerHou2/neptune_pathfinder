...
for i = 1:ld_len
    syms y positive
    eqn = 0 == in.v.gnc.g.p_bank_a.eps*ld_range(i)/2*(y-1) + ...
        (1-in.v.gnc.g.p_bank_a.delta)/sqrt(in.v.gnc.g.p_bank_a.beta*in.v.gnc.g.p_bank_a.R)*log(y);
    soln = solve(eqn,y);
    temp = double(max(soln));
    for j = 1:vi_len
        for k = 1:ga_len
            in_cell{i,j,k} = [ld_range(i),v_init(j),gammas(k),temp];
        end
    end
end

result = NaN(datsize,4);
...
parfor i = 1:datsize
    ...
    this_in.v.gnc.g.p_bank_a.y_f = temp(4);
    ...
    ap = norm(r)/(2 - norm(r)*dot(v,v)/mu);
%     ap = Get_Orb_Params(r,v,mu);
    
    % save to results
    result(i,:) = [LoD, v0, fpa0, ap];
end
...
%% Plot Acceptable Configurations by Apoapsis Constraint
...
ap_tgt = in.v.gnc.g.p_bank_a.tgt_ap;
ap_tol = 400000e3;

out = result;

out(out(:,4)<(ap_tgt-ap_tol),:) = []; % if ap smaller than min acceptable ap, reject
out(out(:,4)>(ap_tgt+ap_tol),:) = []; % if ap larger than max acceptable ap, reject
...
for i = 1:ld_len
    tmp = temp{i};
    scatter3(tmp(:,2)/1e3,tmp(:,3),tmp(:,4)/1e3)
