% aa_define_g_sej_a.m 
%   Define drag modulation aerocapture guidance state structure.
%
...
function [g_sej_a] = aa_define_g_sej_a()
%#codegen
...
p = struct( ...
    ...
    'tj_ini', double(zeros(2,1)), ... % s, initial guesses for jettison times
    ...
    'area_ref', double(zeros(3,1)), ... % m^2, initial (1) and final (2) aerodynamic area reference
    'npole', double(zeros(3,1)), ... % nd, planetary north pole
    'omega', double(zeros(3,1)), ... % rad/s, planetary rotation rate
    'atm_table', double(zeros(1000,2)) ); % md, density vs altitude atm model

%% State data structure
s = struct( ...
    'jettison', logical(zeros(2,1)), ... % nd, jettison flag
    'init_ss', logical(false), ... % nd, second-stage initialization flag
    ...
    'tj_curr',double(0), ... % s, current best estimate jettison time
    'tj_next',double(0), ... % s, next jettison time to try
