% control.m
%   Execute control based on control mode flag - transforms guidance commands
...
function [ ctrl ] = control( calcs, t, si, nav, guid, ctrl )
%#codegen
...
if mod(si-1,ctrl.s.sc_ratio) == 0 % rate limit calls to control
    ...
    switch ctrl.p.bank_mode
        ...
        case 3 % Acceleration- and rate-limited control
            ...
            br = ctrl.s.bank_rate; % current bank rate, rad/s
...
%             end
            % Set bank acceleration to limit
            ba = ctrl.p.bank_accel_lim; % current bank acceleration, rad/s/s
    ...
    switch guid.p.prop_mode
        ...
        case 4 % Aerocapture: n-stage jettison
            % Drag area change
            if (guid.cmd.t_jettison <= t) && (guid.sej_a.s.phase >= uint8(2))
                ctrl.s.jettison = true;
            end
            
        case 6 % Aerocapture: CV drag area
            ctrl.s.jettison = true; % always update area_ref with command
            

        case 7 % Entry: n-stage jettison
