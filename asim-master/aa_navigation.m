% aa_navigation.m
%   Call navigation model based on mode flag - applies errors to truth
%   states
%
...
function [ nav ] = aa_navigation( t, si, calcs, nav_rnd, nav )
%#codegen
...
if mod(si-1,nav.s.sn_ratio) == 0 % rate limit calls to navigation
    ...
    switch nav.p.mode
        ...
        case 3 % scale, bias, & tilt + cd estimation error
            ...
            % Apply scale factor, bias, and tilt
            nav.s.r_pci = ...
                reshape(nav.s.tilt_r,3,3) * (nav.s.r_pci*nav.s.scale(1)+nav.s.bias(1:3));
            nav.s.v_inrtl_pci = ...
                reshape(nav.s.tilt_v,3,3) * (nav.s.v_inrtl_pci*nav.s.scale(2)+nav.s.bias(4:6));
            nav.s.a_sens_pci = ...
                reshape(nav.s.tilt_a,3,3) * (nav.s.a_sens_pci*nav.s.scale(3)+nav.s.bias(7:9));
    ...
    nav.s.v_pf_pcpf = L*nav.s.v_pf_pci;
