...
if s.exit_flag && s.A_mag < p.A_sens_atm
    ...
    while ~cflag
        ...
        [t, y, pflag] = guid_dma_cv_pred(y0, t0, s.ra_next, s.K_dens, p); % run predictor to determine final state
        [s, cflag, cstatus] = guid_dma_cv_corr(t, y, pflag, s, p); % run corrector to adjust jettison time
