...
function [ guid ] = aa_guidance( in, calcs, t, si, nav, guid )
%#codegen
...
if mod(si-1,guid.s.sg_ratio) == 0 % rate limit calls to guidance
    ...
    switch guid.p.bank_mode
        ...
        case 5 % Aerocapture: second order bank angle control - Tiger Hou
            
            % Assign inputs
            guid.bank_a.i.r_pci = nav.s.r_pci/1000; % km, position vector, planet-centered inertial frame
            guid.bank_a.i.v_pci = nav.s.v_inrtl_pci/1000; % km/s, velocity vector, planet-centered inertial frame
            guid.bank_a.i.a_pci = nav.s.a_sens_pci; % m/s^2, sensed acceleration vector, body frame   
            
            % Call bank angle control guidance
            guid.bank_a.s = aa_guid_bank_a( guid.bank_a.i, guid.bank_a.s, guid.bank_a.p, guid.s.init_flag );
            
            % Assign bank command
            cmd_bank = guid.bank_a.s.cmd_bank;
            
        case 6 % Neptune pathfinder probe/main data & guidance code - Tiger Hou
            
            % Assign inputs
            guid.pathfinder.i.t = nav.s.t; % Time, s
            guid.pathfinder.i.R_pci = nav.s.r_pci; % Position vector (PCI), m
            ...
            % Assign bank command
            cmd_bank = guid.pathfinder.s.cmd_bank;
